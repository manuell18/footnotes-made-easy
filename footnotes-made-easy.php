 ...
 */
function fme_enqueue_frontend_styles()
{
	wp_enqueue_style('footnotes-shortcode-styles', plugin_dir_url(__FILE__) . 'css/shortcode.css', array(), filemtime(plugin_dir_path(__FILE__) . 'css/shortcode.css'));
}
add_action('wp_enqueue_scripts', 'fme_enqueue_frontend_styles');
...
class swas_wp_footnotes
{
	...
	function __construct()
	{
		...
		if (!empty($post_array['save_options']) && !empty($post_array['save_footnotes_made_easy_options'])) {
			...
			$footnotes_options['suppress_when_shortcode'] = (array_key_exists('suppress_when_shortcode', $post_array)) ? true : false;
		...
		add_shortcode('footnotes', array($this, 'footnotes_shortcode'));
		add_action('add_meta_boxes', array($this, 'add_footnote_meta_box'));
		add_action('save_post', array($this, 'save_footnote_meta_box'));
	}
	...
	function process($data)
	{
		...
		// Check per-post setting (overrides global)
		if (get_post_meta($post->ID, 'suppress_auto_footnotes', true)) {
			$suppress_auto_footnotes = true;
		}

		if ($display && ! $suppress_auto_footnotes) {
...
<?php
	...
	/**
	 * Shortcode handler for displaying footnotes
	 * 
	 * Usage: [footnotes] - displays all footnotes from current post
	 *        [footnotes ids="1,3,5"] - displays specific footnotes by ID
	 *        [footnotes title="References"] - adds custom title
	 * 
	 * @param array $atts Shortcode attributes
	 * @return string HTML output for footnotes
	 */
	function footnotes_shortcode($atts)
	{
		global $post;

		// Return empty if no post
		if (! $post) {
			return '';
		}
		...
		$atts = shortcode_atts(array(
			'ids' => '', // Specific footnote IDs to display (comma-separated)
			'title' => '', // Custom title for footnotes section
			'show_backlinks' => 'true', // Whether to show back links
			'class' => 'footnotes-shortcode' // Custom CSS class
		), $atts, 'footnotes');

		// Get the post content to extract footnotes
		$content = get_post_field('post_content', $post->ID);

		// Extract footnotes using the same regex as the main process method
		if (! preg_match_all("/(" . preg_quote($this->current_options['footnotes_open'], "/") . ")(.*)(" . preg_quote($this->current_options['footnotes_close'], "/") . ")/Us", $content, $identifiers, PREG_SET_ORDER)) {
			return ''; // No footnotes found
		}

		// Process footnotes similar to main process method
		$footnotes = array();
		...
		for ($i = 0; $i < count($identifiers); $i++) {
			...
			// Handle combining identical notes
			if ($this->current_options['combine_identical_notes']) {
				for ($j = 0; $j < count($footnotes); $j++) {
					if ($footnotes[$j]['text'] === $identifiers[$i]['text']) {
						$identifiers[$i]['use_footnote'] = $j;
		...
		// Filter footnotes if specific IDs requested
		if (! empty($atts['ids'])) {
			$requested_ids = array_map('intval', explode(',', $atts['ids']));
			$filtered_footnotes = array();

			foreach ($requested_ids as $id) {
				if (isset($footnotes[$id - 1])) {
					$filtered_footnotes[] = $footnotes[$id - 1];
				}
			}
		...
		if (! empty($atts['title'])) {
			$output .= '<h3 class="footnotes-title">' . esc_html($atts['title']) . '</h3>';
		}

		$output .= $this->current_options['pre_footnotes'];

		$start = ($start_number !== 1) ? 'start="' . $start_number . '" ' : '';
		$output .= '<ol ' . $start . 'class="footnotes footnotes-shortcode-list">';

		foreach ($footnotes as $key => $value) {
			$output .= '<li id="footnote_shortcode_' . $key . '_' . $post->ID . '" class="footnote"';

			if ('symbol' === $style) {
				$output .= ' style="list-style-type:none;"';
			} elseif ($style !== $this->current_options['list_style_type']) {
				$output .= ' style="list-style-type:' . $style . ';"';
			}

			$output .= '>';

			if ('symbol' === $style) {
				$output .= '<span class="symbol">' . $this->convert_num($key + $start_number, $style, count($footnotes)) . '</span> ';
			}

			$output .= $value['text'];

			// Add backlinks if enabled
			if ('true' === $atts['show_backlinks'] && isset($value['identifiers'])) {
				$output .= '<span class="footnote-back-link-wrapper">';
				foreach ($value['identifiers'] as $identifier) {
					$output .= $this->current_options['pre_backlink'] . '<a href="#identifier_' . $identifier . '_' . $post->ID . '" class="footnote-link footnote-back-link">' . $this->current_options['backlink'] . '</a>' . $this->current_options['post_backlink'];
				}
				$output .= '</span>';
			}
		...
		$output .= '</ol>' . $this->current_options['post_footnotes'];
		$output .= '</div>';
	...
	/**
	 * Modified process method to optionally suppress footnotes at bottom
	 * 
	 * Add this parameter check to the existing process method where footnotes are displayed:
	 */
	function process_with_shortcode_support($data)
	{
		// ... existing code ...

		// Before the footnotes display section, add this check:
		// Check if footnotes should be suppressed due to shortcode usage
		$suppress_auto_footnotes = has_shortcode($data, 'footnotes') && get_post_meta($post->ID, 'suppress_auto_footnotes', true);

		// Display footnotes (modify the existing condition)
		if ($display && ! $suppress_auto_footnotes) {
			// ... existing footnotes display code ...
		}
	...
	/**
	 * Add meta box to post editor for footnote options
	 */
	function add_footnote_meta_box()
	{
		add_meta_box(
			'footnotes-options',
			__('Footnotes Options', 'footnotes-made-easy'),
			array($this, 'footnote_meta_box_callback'),
			array('post', 'page'),
			'side',
			'default'
		);
	 ...
	 * Meta box callback
	 */
	function footnote_meta_box_callback($post)
	{
		wp_nonce_field('footnotes_meta_box', 'footnotes_meta_box_nonce');

		$suppress_auto = get_post_meta($post->ID, 'suppress_auto_footnotes', true);
		$footnote_style = get_post_meta($post->ID, 'footnote_style', true);

		echo '<p>';
		echo '<input type="checkbox" id="suppress_auto_footnotes" name="suppress_auto_footnotes" value="1" ' . checked(1, $suppress_auto, false) . ' />';
		echo '<label for="suppress_auto_footnotes">' . __('Suppress automatic footnotes display (use shortcode instead)', 'footnotes-made-easy') . '</label>';
		echo '</p>';

		echo '<p>';
		echo '<label for="footnote_style">' . __('Footnote Style:', 'footnotes-made-easy') . '</label><br>';
		echo '<select id="footnote_style" name="footnote_style">';
		echo '<option value="">' . __('Default', 'footnotes-made-easy') . '</option>';

		foreach ($this->styles as $key => $label) {
			echo '<option value="' . esc_attr($key) . '" ' . selected($footnote_style, $key, false) . '>' . esc_html($label) . '</option>';
		}
	 ...
	 * Save meta box data
	 */
	function save_footnote_meta_box($post_id)
	{
		if (! isset($_POST['footnotes_meta_box_nonce'])) {
			return;
		}

		if (! wp_verify_nonce($_POST['footnotes_meta_box_nonce'], 'footnotes_meta_box')) {
			return;
		}
		...
		if (isset($_POST['suppress_auto_footnotes'])) {
			update_post_meta($post_id, 'suppress_auto_footnotes', 1);
		} else {
			delete_post_meta($post_id, 'suppress_auto_footnotes');
		}

		if (isset($_POST['footnote_style']) && array_key_exists($_POST['footnote_style'], $this->styles)) {
			update_post_meta($post_id, 'footnote_style', sanitize_text_field($_POST['footnote_style']));
		} else {
